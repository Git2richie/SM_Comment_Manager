...
CREATE TABLE OTHER_CHANNEL_VIDEOS (
    VIDEO_ID STRING,
    TITLE STRING,
    PUBLISHED_AT TIMESTAMP_NTZ
);

CREATE TABLE YT_COMMENTS_STAGE (
    COMMENT_ID STRING,
    VIDEO_ID STRING,
    COMMENT_TEXT STRING,
...
CREATE OR REPLACE SECRET youtube_api_token
  TYPE = GENERIC_STRING
  SECRET_STRING = 'YOUR_YOUTUBE_API_TOKEN_HERE';
4. Deployment
...
📖 How to Use
Sync Video List: Enter a YouTube Channel ID and click "Sync." This pulls the latest video directory.
...
[ ] Bulk Publishing: Sending approved replies directly back to YouTube via API.
